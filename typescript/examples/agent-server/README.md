...
TypeScript port of `python/examples/agent-server`. A production-like ERC-8183 provider agent that searches DuckDuckGo for blockchain news and stores deliverables locally (or on IPFS via Pinata).

This is the SDK's **HTTP serving reference**: the factory (`createErc8183Server`, the funded-job poll loop, `/negotiate` + job routes) lives in `src/erc8183Server.ts` - it is example code, not SDK API. Copy the directory and own it; the SDK underneath ships only the headless primitives (`ERC8183JobOps`, `fundedJobWatcher`, `NegotiationHandler`, `SlidingWindowLimiter`). For the recommended agent-facing surface (A2A), see `../a2a-agent`.

> **Example wiring:** This server uses `ERC8183Config.fromEnv()`, so `WALLET_KIND=twak` selects the TypeScript `TWAKProvider`; the default remains the EVM keystore backend. Altana requires explicit session construction and is demonstrated separately in `../altana`.
>
> **Zero HTTP framework.** The serving layer is a ~100-line `node:http` router (`src/http.ts`), matching the `examples/x402` precedent. Swap it for Hono/Fastify/Express in a real service - the handlers only touch `IncomingMessage`/`ServerResponse`.
...
ERC-8183 uses the **OptimisticPolicy**: silence approves after the dispute window; a client-raised dispute must reach a whitelisted-voter quorum to flip the verdict to REJECT. There is no auto-settle - run `scripts/settle.ts`.
