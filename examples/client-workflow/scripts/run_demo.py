"""
Demo Flow — APEX (ERC-8183) Client.

Built with bnbagent-sdk (see: bnbagent-sdk/examples/client_workflow.py).

A client script that drives the full APEX (ERC-8183) lifecycle:
...
# SDK imports — same as bnbagent-sdk/examples/client_workflow.py
from bnbagent import APEXClient, APEXStatus, APEXEvaluatorClient, NonceManager, TESTNET_CONFIG, load_erc20_abi
from bnbagent.apex.client import get_default_expiry

# ---------------------------------------------------------------------------
# Configuration
# ---------------------------------------------------------------------------

RPC_URL = os.environ.get("RPC_URL") or TESTNET_CONFIG["rpc_url"]
ERC8183_ADDRESS = os.environ.get("ERC8183_ADDRESS") or TESTNET_CONFIG["erc8183_contract"]
APEX_EVALUATOR_ADDRESS = os.environ.get("APEX_EVALUATOR_ADDRESS") or TESTNET_CONFIG["apex_evaluator"]
...
PAYMENT_TOKEN_ADDRESS = os.environ.get("PAYMENT_TOKEN_ADDRESS") or TESTNET_CONFIG["payment_token"]
...
def discover_agent_from_8004(agent_id: str) -> tuple[str, str]:
    ...
    # Parse agent URI to get endpoint
    ...
    if agent_uri:
        ...
        try:
            if agent_uri.startswith("data:application/json;base64,"):
                json_str = base64.b64decode(agent_uri.split(",", 1)[1]).decode("utf-8")
            elif agent_uri.startswith("data:application/json,"):
                json_str = agent_uri.split(",", 1)[1]
            else:
                json_str = agent_uri
...
async def main():
    ...
    # ── Web3 + SDK setup (same as client_workflow.py) ──
...
""")
    ...
    #   1. IPFS via Pinata (if PINATA_JWT configured)
