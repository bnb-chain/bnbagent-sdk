...
BNBAgent SDK is the **protocol meta-capability layer** for building on-chain AI agents on BNB Chain. It provides wallet management, signing policy, off-chain storage abstraction, and built-in support for the following protocols:
...
- **x402** - HTTP 402 micropayment signing (EIP-3009) with budget tracking and delegated payers.

Wallet signing and off-chain storage are abstracted behind provider interfaces, making the SDK backend-agnostic. Each protocol package is independently importable - there is no framework, registry, or required composition root.
...
The boundary criterion, in one sentence: **protocol-defined / security-critical / needed by every consumer → SDK; interprets a product's config or carries a product opinion → the layer above.** Capabilities migrate downward item by item when they prove universal (e.g. `to_raw`/`from_raw` were lifted into `bnbagent.utils` from a downstream consumer) - never as a wholesale merge.

**Serving surfaces are deliberately NOT in the SDK.** How an agent faces the world - A2A, MCP, plain HTTP - is an application choice. The recommended direction is **A2A first, MCP second**; the SDK's role is limited to:

- the **registration side**: `AgentEndpoint.a2a()` / `AgentEndpoint.mcp()` encode each protocol's ERC-8004 registration convention;
- the **headless primitives** every serving form shares: `ERC8183JobOps`, `funded_job_watcher`, `NegotiationHandler`, `SlidingWindowLimiter`.

Reference serving implementations live in `examples/` (`a2a-agent/` for A2A, `agent-server/` for HTTP) as copy-and-own code, not SDK API.
...
Arrows point **downward** - upper layers depend on lower layers, never the reverse.

## Code Map

The Python and TypeScript packages implement the same protocol layers with language-specific naming and runtime plumbing. The detailed map below starts with Python; the TypeScript map calls out the corresponding source and the additional wallet backends now available there.

### Python: `python/bnbagent/`
...
| `capabilities.py` | Open capability-string registry (`sign.*`, `intents.*`, `x402.pay`, and vendor-defined values) |
...
| `rate_limit.py` | `SlidingWindowLimiter` (+ `RateLimitExceeded`) - transport-agnostic per-key throttle; serving layers map the exception to their protocol's rejection, such as HTTP 429 |

#### `bnbagent/networks/` - Deployment Registry

Canonical chain IDs, contract addresses and payment-token EIP-712 metadata (`BNB_CHAIN_ADDRESSES`). Single source of truth for downstream layers.
...
`pytest` + `pytest-mock` + `pytest-asyncio`. Tests mock web3 and external services; no live chain calls in CI. The HTTP serving tests live with the HTTP example (`examples/agent-server/tests/`), not in the SDK suite.

### TypeScript: `typescript/`

| Path | Purpose |
| --- | --- |
| `src/index.ts` | Tier 1 public API, including the relay-observability errors and transaction tuning controls |
| `src/core/` | viem clients, nonce management, paymaster transport, transaction sending, and multi-RPC relay confirmation |
| `src/erc8004/`, `src/erc8183/`, `src/x402/` | Identity, commerce, negotiation/quote verification, and payment protocol modules |
| `src/wallets/` | EVM keystore, TWAK CLI, and Altana EIP-7702/session-key providers |
| `src/storage/`, `src/signing/`, `src/networks/`, `src/utils/` | Storage, signing policy, deployment registry, and shared utilities |
| `examples/` | Client/voter flows, A2A and HTTP serving references, plus Altana examples |
| `tests/` | Vitest suite; chain interactions use mocked transports except explicitly gated E2E examples |
...
from bnbagent import (
    ...
    TransactionPendingError, ERC8004PartialRegistrationError,
    ...
    set_default_receipt_timeout, get_default_receipt_timeout,
)
```
...
The main entry point exports the essentials, including all SDK error classes and the EVM, TWAK, and Altana providers. Full module surfaces are available through the package subpaths.
...
import { ERC8183JobOps, NegotiationHandler } from "@bnbagent/sdk/erc8183";
```
...
The per-network RPC pins exist because a single process may touch several networks at once; a single shared `RPC_URL` would silently apply to the wrong chain. Contract-address overrides are applied by each module's own config loader - keeps each module's env surface self-contained and obvious from the prefix.
...
Payment token address is NOT configurable - it is immutable on the Commerce kernel and fetched at runtime via `ERC8183Client.payment_token`.

Configs support the convenience pattern: pass `private_key` + `wallet_password` and the config auto-wraps them into an `EVMWalletProvider`, then **clears both the plaintext key and password** from the config object (the provider keeps its own password copy).
...
- **The SDK ships no serving runtime.** No HTTP/A2A/MCP server lives under `bnbagent/`; `fastapi` is not an SDK dependency. Serving forms are examples or downstream applications.
- **Capability declaration cannot drift from behavior.** `sign.*` capabilities are auto-derived from method overrides; never override a `sign_*` method to raise.
...
- **Payment token is dynamically fetched.** It is never part of `NetworkConfig` or `ERC8183Config` because it is an immutable property of the deployed Commerce kernel.
...
The watcher is signer-free detection (it never submits or settles itself); `ERC8183JobOps` constructed with only a `provider_address` (no wallet) is the SDK-level keyless read path - any signing call raises. The SDK does NOT auto-settle: settle is permissionless and runs as a separate operator script (see `python/examples/agent-server/scripts/settle.py`).
...
│   └── RpcRangeLimitError - RPC log/range query rejected by a node limit
...
├── TransactionPendingError - broadcast transaction still awaiting a receipt
├── RelaySubmissionUnverifiedError - relay hash unseen by chain RPCs (TypeScript)
│   └── RelayFallbackFailedError - self-pay fallback could not be broadcast (TypeScript)
├── RelayRejectedError   - relay rejected or ambiguously failed a submission (TypeScript)
└── ERC8004PartialRegistrationError - identity minted but metadata/URI phase failed
```

(`bnbagent.utils.RateLimitExceeded` and the wallet/x402 error families are deliberately outside this hierarchy - they are utility/capability errors, not protocol-flow errors.)

## Extension Points

- **Custom WalletProvider** - subclass `WalletProvider` to support HSMs, multisig, or MPC signers; declare capabilities by overriding the matching methods.
...
- **Custom serving surface** - compose `ERC8183JobOps` + `funded_job_watcher`
  - `NegotiationHandler` behind any transport; register it with `AgentEndpoint.a2a()` / `.mcp()`. The examples are the templates.
...
| TypeScript core | Node.js ≥ 20, `viem ^2.21`, `dotenv ^16.4`, `@noble/hashes`, `@noble/ciphers` |
| TypeScript optional wallet | `@altananetwork/sdk >=0.3.3 <0.6.0` |

(`fastapi`/`uvicorn` are dependencies of the HTTP/A2A _examples_, not the SDK.)
