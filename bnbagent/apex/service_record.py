...
class ServiceRecord:
    ...
    negotiation: Optional[NegotiationData] = None
    ...
    def compute_hashes(self) -> dict:
        ...
        if self.negotiation:
            if self.negotiation.request:
                negotiation_request_hash = ensure_0x(Web3.keccak(
                    text=json.dumps(self.negotiation.request, sort_keys=True, separators=(",", ":"))
                ).hex())
                self.negotiation.request_hash = negotiation_request_hash
            if self.negotiation.response:
                negotiation_response_hash = ensure_0x(Web3.keccak(
                    text=json.dumps(self.negotiation.response, sort_keys=True, separators=(",", ":"))
                ).hex())
        ...
        if self.service and self.service.response_content:
            service_response_hash = ensure_0x(Web3.keccak(text=self.service.response_content).hex())
