"""
OOv3Evaluator contract interaction client.

Provides typed Python methods for interacting with the UMA OOv3-based evaluator:
...
def _load_oov3_evaluator_abi() -> list:
    abi_path = Path(__file__).parent / "abis" / "OOv3Evaluator.json"
    try:
...
class OOv3EvaluatorClient:
    """
    Python client for the OOv3Evaluator contract.
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_rate_limit and attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        f"[OOv3EvaluatorClient] Rate limited, retry {attempt + 1}/{MAX_RETRIES} "
                        f"in {delay:.1f}s"
                    )
                    time.sleep(delay)
                else:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_rate_limit and attempt < MAX_RETRIES - 1:
                    delay = RETRY_BASE_DELAY * (2 ** attempt)
                    logger.warning(
                        f"[OOv3EvaluatorClient] Rate limited (read), retry {attempt + 1}/{MAX_RETRIES} "
                        f"in {delay:.1f}s"
                    )
                    time.sleep(delay)
                else:
    ...
    def get_acp_address(self) -> str:
        """Get the ACP contract address."""
        return self._call_with_retry(self.contract.functions.acp())
