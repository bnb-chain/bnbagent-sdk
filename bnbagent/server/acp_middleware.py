"""
ACPMiddleware — FastAPI middleware for EIP-8183 job verification.
...
from ..acp_client import ACPStatus
...
class ACPMiddleware:
    """
    FastAPI/Starlette middleware for EIP-8183 job verification.
    ...
    def __init__(
        ...
        Args:
            ...
            job_ops: ACPJobOps instance for verification
        ...
        logger.info(
            f"[ACPMiddleware] Initialized with skip_paths={self._skip_paths}"
        )
    ...
    async def __call__(self, scope, receive, send):
        ...
        logger.info(
            f"[ACPMiddleware] Verified job {job_id} "
...
def create_acp_middleware(
    job_ops: "ACPJobOps",
    skip_paths: Optional[List[str]] = None,
):
    """
    Factory function to create ACPMiddleware for use with app.add_middleware().

    Example:
        from bnbagent.server import create_acp_middleware, ACPJobOps

        job_ops = ACPJobOps(...)
        app.add_middleware(ACPMiddleware, job_ops=job_ops)
    """
