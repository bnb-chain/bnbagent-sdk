"""
EIP-8183 Agentic Commerce Protocol contract interaction client.
...
class ACPStatus(IntEnum):
    """EIP-8183 Job status enum."""
    NONE = 0
    OPEN = 1
    FUNDED = 2
    SUBMITTED = 3
...
def _load_acp_abi() -> list:
    abi_path = Path(__file__).parent / "abis" / "ACP.json"
    try:
...
class ACPClient:
    """
    Python client for the EIP-8183 Agentic Commerce Protocol (AgenticCommerceUpgradeable) contract.
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_rate_limit and attempt < MAX_RETRIES - 1:
                    ...
                    logger.warning(
                        f"[ACPClient] Rate limited, retry {attempt + 1}/{MAX_RETRIES} "
                        f"in {delay:.1f}s"
                    )
                    time.sleep(delay)
                else:
        ...
        for attempt in range(MAX_RETRIES):
            try:
                ...
                if is_rate_limit and attempt < MAX_RETRIES - 1:
                    delay = RETRY_BASE_DELAY * (2 ** attempt)
                    logger.warning(
                        f"[ACPClient] Rate limited (read), retry {attempt + 1}/{MAX_RETRIES} "
                        f"in {delay:.1f}s"
                    )
                    time.sleep(delay)
                else:
    ...
    def get_job(self, job_id: int) -> Dict[str, Any]:
        ...
        return {
            ...
            "budget": raw[4],
            "expiredAt": raw[5],
            "status": ACPStatus(raw[6]),
            "deliverable": raw[7],
            "description": raw[8],
        }

    def get_job_status(self, job_id: int) -> ACPStatus:
    ...
    def payment_token(self) -> str:
        """Get the payment token address configured in the ACP contract."""
