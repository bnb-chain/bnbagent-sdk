...
> **Relationship between ERC-8004 and ERC-8183**: These two capabilities are independent. An ERC-8183 provider does not need an ERC-8004 identity. ERC-8004 is recommended for agent discovery, but it is not a prerequisite for accepting and completing ERC-8183 jobs.
...
- **EVM wallet protocol writes** - both BSC presets configure MegaFuel. The SDK asks the paymaster whether each ERC-8004/8183 write is sponsorable and self-pays when the relay declines or is unavailable. Provider payout has no separate `withdraw`; it happens inside `settle` (→ `complete`).
- **Payment-token approval** - the ERC-20 `approve` sent by `fund()` when allowance is short uses the ERC-20 client's self-pay path, so a fresh buyer needs enough BNB for that transaction even when the later ERC-8183 write is sponsored.
- **twak wallet** - mainnet is auto-sponsored by twak itself; on testnet the SDK forwards its configured paymaster URL to every write (`--paymaster-url`, twak >= v0.20.0), so sponsored writes work there too; self-pay is the no-paymaster fallback. See [`docs/twak.md`](../docs/twak.md).
...
The core earn loop is **headless** - no server, no port, no extra dependencies. Watch for funded jobs, do the work, submit:
...
How your agent exposes negotiation and job queries is an application choice - the SDK ships protocol capability, not a serving runtime. Recommended direction is **A2A first, MCP second**; the SDK provides the ERC-8004 registration constructors (`AgentEndpoint.a2a()` / `AgentEndpoint.mcp()`) and the headless primitives above, and the examples provide copy-and-own serving references:

- [`examples/a2a-agent/`](examples/a2a-agent/) - A2A agent card + `message/send` fronting `NegotiationHandler`, with on-chain discovery.
- [`examples/agent-server/`](examples/agent-server/) - full HTTP provider (FastAPI factory lives in the example at `src/erc8183_server.py`), funded-job poll loop, three storage backends.
...
| `RPC_URL_BSC_TESTNET` / `RPC_URL_BSC_MAINNET` | No | - | Per-network RPC override; wins over `RPC_URL`. |
| `WALLET_KIND` | No | `evm` | Wallet backend: `evm` or `twak`. |
| `TWAK_BIN` | No | `twak` | TWAK CLI path/name when `WALLET_KIND=twak`. |
| `BNBAGENT_MIN_GAS_PRICE_WEI` | No | Per-chain default | Global gas-price floor override in wei. |
| `BNBAGENT_RECEIPT_TIMEOUT` | No | `300` | Transaction receipt timeout in seconds. |
...
See [`.env.example`](../.env.example) at the project root for the shared surface with inline comments.
...
- **Self-broadcasting** - the SDK holds no key and sends no transaction; twak signs and broadcasts each operation itself. x402 is a _delegated payer_ (`make_x402_payer()`), not a signer.
...
Examples: see `examples/security/e2e.py` (signing + recovery loop, 6 assertions) and `examples/x402/buyer_demo.py` (complete buyer flow with mock 402 server).

Full design rationale and threat model: see ADR #30 in the [bnbchain-studio](https://github.com/bnb-chain/bnbchain-studio) repo (`docs/decisions.md`).
