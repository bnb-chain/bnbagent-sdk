...
| REQ-4 | `x402 request --header <k:v>` (repeatable) or `--emit-envelope` (emit only the `X-PAYMENT` header and let the caller send it over its own session) | P2 (non-blocking) | mixed-auth endpoints (`Authorization` + 402): `TwakX402Payer` owns the whole HTTP round-trip today, so endpoints needing extra headers cannot be paid via twak; studio fail-fasts with an equivalent flow (`--initial-usd 0` + topup + allocate) | 🙅 open (raised from studio acceptance, 2026-06-12) |
...
| S-10 | `wallet sign-message --chain` should accept the same chain keys as the rest of the CLI — today it is a *key-family* selector (help: "e.g., ethereum, solana") that accepts `bsc` but rejects `bsctestnet` with `Unsupported chain` (field-verified v0.19.0; re-verified unchanged on v0.19.1), unlike every `erc8183` command | EIP-191 is chain-agnostic and the address is identical on both BNB networks, so the SDK always passes `bsc` for signing (with an ecrecover self-check) |
| S-11 | `wallet sign-message --message` hex-decoded `0x`-prefixed messages and signed the raw bytes while the `digest` field stayed text-computed (self-contradicting output; every ERC-8183 negotiation-hash signature affected) — ✅ fixed in v0.19.1 ("input is always text"). Upstream FYI: the bytes-signing was present since <= v0.18.0 (the v0.19.0 `digest` field merely exposed it) — historical signatures over hex-shaped messages must be **re-signed**, not re-verified | SDK passes messages verbatim and requires twak >= v0.19.1; the ecrecover self-check (which caught this live) names this failure mode in its error |
...
| v0.19.1 | 2026-06-12 (3) | Upstream shipped the S-11 fix same-day ("input is always text"). Field-verified: a `0x`-shaped message now recovers to the wallet address under TEXT semantics (bytes-semantics recovery no longer matches), and the `digest` field is consistent with the signature. S-10 re-verified **unchanged** (`--chain bsctestnet` still rejected — the `bsc` pin stays). SDK response: minimum supported version raised 0.19.0 → 0.19.1 (0.19.0's bad signer must not stay silently usable), the v0.19.0-era hex-encoding workaround reverted to verbatim text, the self-check error now names S-11 + the upgrade path, and two regression nets added (wallet-conformance hex-shaped-message test, e2e smoke step 0b). Upstream FYI recorded: the bytes-signing existed since <= v0.18.0 (the v0.19.0 `digest` field merely exposed it) — re-sign, don't re-verify, any stored hex-shaped-message signatures. The SDK stores none: quote signatures are ephemeral (TTL 300s). |
